...
public class TelaAnagrama extends AppCompatActivity {
    ...
    private  List<String> listaDeDicas;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Inicializa os botões antes de chamá-los nos listeners
        ...
        btnEmbaralhar.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (minhaLista != null && !minhaLista.isEmpty()) {
                    ...
                    Log.e(TAG, "A lista de palavras está vazia ou não foi carregada.");
                }
        ...
        // Inicializa o WordsManager
        wordsManager = new WordsManager(this);

        // Busca a lista de palavras da API
        wordsManager.fetchWords(new WordsCallback() {
            @Override
            public void onSuccess(List<String> words, List<String> hints) {
                minhaLista = words;
                // Se precisar da lista de dicas, você pode armazená-la em outra variável
                listaDeDicas = hints;
                novoJogo(); // Inicia um novo jogo quando a lista de palavras é carregada
            }

            @Override
            public void onError(Exception e) {
                Log.e(TAG, "Erro ao buscar palavras da API: " + e.getMessage());
                // Pode ser útil inicializar a lista com palavras padrão caso haja um erro
            }
        });
    }
    ...
    List<Integer> palavrasRepetidas = new ArrayList<>();
    ...
    public String listaNomes() {
        ...
        if (arquivos.contains("pontos")) {
            ...
            posicaoLista = palavraAleatoria.nextInt(minhaLista.size());

            while (palavrasRepetidas.contains(posicaoLista)) {
                posicaoLista = palavraAleatoria.nextInt(minhaLista.size());
            }

            palavrasRepetidas.add(posicaoLista);
    ...
    public String listaDica() {
        List<String> minhaListaDicas = listaDeDicas;

        // Verifica se o índice está dentro dos limites da lista de dicas
        if (posicaoLista < minhaListaDicas.size()) {
            String dica = minhaListaDicas.get(posicaoLista);
            txtDica.setText("A dica é: " + dica);
        } else {
            // Caso não haja uma dica correspondente, exibe uma mensagem padrão
            txtDica.setText("A dica não está disponível.");
        }
    ...
    private void salvarPontos(){
        ...
        if (arquivos.contains("pontos")){
            ...
            if(qtdPontos > 3){
                Intent telaVitoria = new Intent(getApplication(), TelaInstrucoes2.class);
                startActivity(telaVitoria);
            }
